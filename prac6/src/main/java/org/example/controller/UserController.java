...
public class UserController {
    ...
    public ResponseEntity<?> update(@PathVariable(name="id") long id, @RequestBody User user) {
        ...
        var i = userFromDb.getId();
        return userService.update(user, i) == true
    ...
    public ResponseEntity<List<User>> listUsersByName(@PathVariable(name="name") String name)
    {
        var users = userService.findUsersByLogin(name);
        return users != null
                ? new ResponseEntity<>(users, HttpStatus.OK)
                : new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
